        ...
        if records:
            ...
            for r in records:
                try:
                    ...
                    logger.error(
                        "Gagal mem-parsing harga untuk %s: %s",
                        r.get("komoditas", ""),
                        e,
                    )
            ...
            if history_records:
                try:
                    await asyncio.to_thread(save_price_history_batch, history_records)
                except Exception as e:
                    logger.error("Gagal menyimpan histori harga secara batch: %s", e)
