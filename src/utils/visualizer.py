...
def generate_price_chart(komoditas: str, data: list[dict[str, Any]]) -> str | None:
    ...
    try:
        ...
        for item in data_sorted:
            ...
            full_name = (
                DAERAH_LIST.get(item["kode_daerah"], item["kode_daerah"])
                .replace("Kab. ", "")
                .replace("Kota ", "")
            )
        ...
        # Sanitize komoditas name to prevent path traversal
        safe_komoditas = re.sub(r"[^a-z0-9_]", "_", komoditas.lower())
        file_path = temp_dir / f"chart_{safe_komoditas}.png"
