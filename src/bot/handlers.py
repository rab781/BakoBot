...
async def termurah_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if not data:
            safe_komoditas_name = escape_markdown(komoditas.upper(), version=1)
            await loading_msg.edit_text(
                f"❌ Data untuk komoditas *{safe_komoditas_name}* tidak ditemukan.\n"
        ...
        with open(chart_path, "rb") as photo:
            await update.message.reply_photo(
                photo=photo,
                caption=f"📊 Peta Harga Lintas Daerah: *{safe_komoditas_name}*\n\n"
